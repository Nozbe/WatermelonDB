...
import { schemaMigrations, createTable, addColumns, dropTable, dropColumns, addIndex, removeIndex } from '@nozbe/watermelondb/Schema/migrations'
...
- `dropTable({ table: 'table_name' })` - drops an entire table and all its data
- `dropColumns({ table: 'table_name', columns: ['column1', 'column2'] })` - drops one or multiple columns from an existing table (requires SQLite 3.35.0+)
- `addIndex({ table: 'table_name', column: 'column_name' })` - adds an index to a specific column
- `removeIndex({ table: 'table_name', column: 'column_name' })` - removes an index from a specific column
- Other types of migrations (e.g. renaming tables and columns) are not yet implemented. See [`migrations/index.js`](https://github.com/Nozbe/WatermelonDB/blob/master/src/Schema/migrations/index.js). Please contribute!
...
The `dropColumns` migration step requires SQLite 3.35.0 or later (released in March 2021). If you're using an older version of SQLite, you'll need to use `unsafeExecuteSql()` to manually handle column dropping, or upgrade your SQLite version.
