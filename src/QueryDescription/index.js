...
import { unique, unnest } from '../utils/fp'
...
const extractClauses: (Clause[]) => QueryDescription = (clauses) => {
  const query = { select: [], where: [], joinTables: [], nestedJoinTables: [], sortBy: [] }
  clauses.forEach((clause) => {
    const { type } = clause
    switch (type) {
      case 'select':
        query.select.push(clause)
        break
      case 'where':
      case 'and':
      case 'or':
...
export function getSelectedColumns(query: QueryDescription): ColumnName[] {
  return unique(unnest(query.select.map(({ columns }) => columns)))
}
