...
type ActionQueueItem<T> = {
  work: (arg1: ActionInterface) => Promise<T>
...
export default class ActionQueue implements ActionInterface {
  ...
  enqueue<T>(work: (arg1: ActionInterface) => Promise<T>, description?: string): Promise<T> {
    ...
    return new Promise(
      (resolve: (result: Promise<never>) => void, reject: (error?: any) => void) => {
