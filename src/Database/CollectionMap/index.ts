...
export default class CollectionMap {
  map: Partial<Record<TableName<any>, Collection<any>>>;

  constructor(db: Database, modelClasses: typeof Model[]) {
    this.map = (Object.create(null) as any)
  ...
  get<T extends Model>(tableName: TableName<T>): Collection<T> {
    return this.map[tableName] || null as any;
  }
}
