...
type RecordState = Partial<Record<ColumnName, Value>>;

const getRecordState: (arg1: Model, arg2: ColumnName[]) => RecordState = (record, columnNames) =>
  // `pickAll` guarantees same length and order of keys!
  pickAll(columnNames, record._raw)

// Invariant: same length and order of keys!
const recordStatesEqual = (left: RecordState, right: RecordState): boolean => identicalArrays(values(left), values(right))
...
export default function subscribeToQueryWithColumns<Record extends Model>(
  ...
  const [subscribeToSource, asyncSource] = canEncodeMatcher(query.description)
    ? [
        (observer: (recordsOrStatus: boolean | Array<Record>) => void) =>
          subscribeToSimpleQuery(query, observer, true),
        false,
      ]
    : [
        (observer: (recordsOrStatus: boolean | Array<Record>) => void) =>
          subscribeToQueryReloading(query, observer, true),
        false,
      ]
  // NOTE:
  ...
  const sourceUnsubscribe = subscribeToSource(function observeWithColumnsSourceChanged(recordsOrStatus): undefined {
    ...
    const records: Record[] = recordsOrStatus as any
    const shouldEmit =
