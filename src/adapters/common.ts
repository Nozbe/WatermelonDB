...
import { sanitizedRaw, DirtyRaw } from '../RawRecord'

export type DirtyFindResult = RecordId | DirtyRaw | null | undefined;
...
export function sanitizeFindResult(dirtyRecord: DirtyFindResult, tableSchema: TableSchema): CachedFindResult {
  return dirtyRecord && typeof dirtyRecord === 'object'
    ? sanitizedRaw(dirtyRecord, tableSchema)
    : dirtyRecord
}

export function sanitizeQueryResult(dirtyRecords: DirtyQueryResult, tableSchema: TableSchema): CachedQueryResult {
  return dirtyRecords.map(dirtyRecord =>
    typeof dirtyRecord === 'string' ? dirtyRecord : sanitizedRaw(dirtyRecord, tableSchema),
  )
}

export function devSetupCallback(result: any): void {
  if (result.error) {
    logger.error(
