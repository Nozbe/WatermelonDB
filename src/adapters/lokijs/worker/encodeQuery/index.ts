...
export type LokiRawQuery = any | typeof undefined;
type LokiOperator = '$aeq' | '$eq' | '$gt' | '$gte' | '$lt' | '$lte' | '$ne' | '$in' | '$nin' | '$between' | '$regex';
type LokiKeyword = LokiOperator | '$and' | '$or';

export type LokiJoin = {
  table: TableName<any>;
  query: LokiRawQuery;
  mapKey: ColumnName;
  joinKey: ColumnName;
...
const weakNotNull = { $not: { $aeq: null } } as const
...
const columnCompRequiresColumnNotNull: Partial<Record<Operator, boolean>> = {
  gt: true,
  gte: true,
  lt: true,
  lte: true,
...
const encodeWhereDescription: (arg1: WhereDescription) => LokiRawQuery = ({ left, comparison }) => {
  ...
  const colName: string | null | undefined = (right as any).column
...
const encodeCondition: (arg1: QueryAssociation[]) => (arg1: Clause) => LokiRawQuery = associations => clause => {
  switch (clause.type) {
    case 'and':
...
const encodeConditions: (arg1: QueryAssociation[], arg2: Where[]) => LokiRawQuery[] = (
  associations,
  conditions,
) => conditions.map(encodeCondition(associations))

const encodeAndOr = (op: LokiKeyword) => (associations: QueryAssociation[], clause: And | Or): LokiRawQuery => {
  const conditions = encodeConditions(associations, clause.conditions)
  // flatten
  return conditions.length === 1 ? conditions[0] : { [op]: conditions };
}

const encodeAnd: (arg1: QueryAssociation[], arg2: And) => LokiRawQuery = encodeAndOr('$and')
const encodeOr: (arg1: QueryAssociation[], arg2: Or) => LokiRawQuery = encodeAndOr('$or')
...
const encodeRootConditions: (arg1: QueryAssociation[], arg2: Where[]) => LokiRawQuery = (
  associations,
  conditions,
...
const encodeJoin = (associations: QueryAssociation[], on: On): LokiRawQuery => {
  ...
  return {
    $join: {
      table,
      query: encodeRootConditions(associations, (conditions as any)),
