...
const isIDBAvailable = (onQuotaExceededError?: ((error: Error) => void) | null) => {
  return new Promise((resolve: (result: Promise<boolean> | boolean) => void) => {
    ...
    checkRequest.onsuccess = (e: any) => {
      const db: IDBDatabase = e.target.result
      db.close()
      resolve(true)
    }
    checkRequest.onerror = (event: any) => {
      const error: Error | null | undefined = event?.target?.error
...
async function getLokiAdapter(options: LokiAdapterOptions) {
  const {
...
export async function newLoki(options: LokiAdapterOptions): Promise<Loki> {
  ...
  await new Promise((resolve: (result?: Promise<undefined> | undefined) => void, reject: (error?: any) => void) => {
    loki.loadDatabase({}, (error: any) => {
...
export async function deleteDatabase(loki: Loki): Promise<void> {
  await new Promise((resolve: (result?: Promise<undefined> | undefined) => void, reject: (error?: any) => void) => {
    ...
    loki.close(() => {
      loki.deleteDatabase({}, (response: any) => {
