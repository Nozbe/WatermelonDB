...
function performJoinsImpl(query: LokiRawQuery, performer: QueryPerformer): LokiRawQuery {
  if (!query) {
    ...
    return { [(join.joinKey as string)]: { $in: matchingIds } };
  } else if (query.$and) {
    return { $and: query.$and.map((clause: any) => performJoinsImpl(clause, performer)) }
  } else if (query.$or) {
    return { $or: query.$or.map((clause: any) => performJoinsImpl(clause, performer)) }
  }
  return query
}
