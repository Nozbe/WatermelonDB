...
import getSyncChanges, { MigrationSyncChanges } from '../../Schema/migrations/getSyncChanges'
...
export async function getLastPulledAt(database: Database, useSequenceIds: boolean = false): Promise<Timestamp | null | undefined | string | null | undefined> {
  const lastPulledAt = parseInt((await database.adapter.getLocal(lastPulledAtKey) as any), 10) || null
...
export async function setLastPulledAt(
  ...
  const timeValue = useSequenceIds ? Math.floor(decodeTime(timestamp as string) / 1000) : timestamp
  await database.adapter.setLocal(lastPulledAtKey, `${timeValue}`)

  if (useSequenceIds) {
    await setLastSequenceId(database, timestamp as string);
  }
}

async function getLastSequenceId(database: Database): Promise<string | null | undefined> {
...
export async function getLastPulledSchemaVersion(database: Database): Promise<SchemaVersion | null | undefined> {
  return parseInt((await database.adapter.getLocal(lastPulledSchemaVersionKey) as any), 10) || null
}

export async function setLastPulledSchemaVersion(database: Database, version: SchemaVersion): Promise<void> {
...
export async function getMigrationInfo(
  database: Database,
  log?: SyncLog | null,
  lastPulledAt?: Timestamp | null | string,
  migrationsEnabledAtVersion?: SchemaVersion | null,
  ...
  if (areMigrationsEnabled) {
    ...
    invariant(
      migrationsEnabledAtVersion >= (migrations?.minVersion || 0),
