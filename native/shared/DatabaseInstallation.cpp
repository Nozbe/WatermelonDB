...
void Database::install(jsi::Runtime *runtime) {
    ...
    createMethod(rt, globalObject, "nativeWatermelonCreateAdapter", 1, [runtime](jsi::Runtime &rt, const jsi::Value *args) {
        ...
        createMethod(rt, adapter, "queryAsArray", 3, [database](jsi::Runtime &rt, const jsi::Value *args) {
            ...
            return runBlock(rt, [&]() { return database->queryAsArray(tableName, sql, arguments); });
        });
        createMethod(rt, adapter, "queryJSON", 3, [database](jsi::Runtime &rt, const jsi::Value *args) {
            assert(database->initialized_);
            jsi::String tableName = args[0].getString(rt);
            jsi::String sql = args[1].getString(rt);
            jsi::Array arguments = args[2].getObject(rt).getArray(rt);
        ...
        createMethod(rt, adapter, "unsafeLoadFromSync", 2, [database](jsi::Runtime &rt, const jsi::Value *args) {
             assert(database->initialized_);
             auto changeSet = args[0].getObject(rt);
             auto schema = args[1].getObject(rt);

             return runBlock(rt, [&]() {
                 database->unsafeLoadFromSync(changeSet, schema);
                 return jsi::Value::undefined();
             });
         });
        createMethod(rt, adapter, "unsafeLoadFromSyncJSON", 2, [database](jsi::Runtime &rt, const jsi::Value *args) {
             assert(database->initialized_);
             auto changeSet = args[0].getString(rt);
             auto schema = args[1].getObject(rt);

             return runBlock(rt, [&]() {
                 database->unsafeLoadFromSyncJSON(changeSet.utf8(rt), schema);
                 return jsi::Value::undefined();
             });
         });
